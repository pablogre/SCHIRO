...
###################################################################################################################
# ============================================================================
# PASO 2: RUTA PARA EMITIR NOTA DE CRÉDITO
# Agregar en app.py DESPUÉS de la función anular_factura
# ============================================================================
...
def emitir_nota_credito(factura_id):
    ...
    try:
        ...
        punto_venta = PUNTO_VENTA  # Variable global de configuración
...
def autorizar_comprobante_afip(comprobante_id, tipo_comprobante='factura'):
    ...
    try:
        ...
        if tipo_comprobante == 'factura':
            ...
            # Mapear tipo de NC según factura
            mapeo_nc = {
                1: 3,   # Factura A → NC A
                6: 8,   # Factura B → NC B
                11: 13, # Factura C → NC C
            ...
            tipo_cbte_afip = mapeo_nc.get(factura_original.tipo_comprobante)
            ...
            cbtes_asoc = [{
                ...
                'Nro': int(numero_str),
                'Cuit': factura_original.cliente.cuit if factura_original.cliente and factura_original.cliente.cuit else None
            }]
        ...
        if comprobante.cliente_id:
            ...
            if cliente and cliente.cuit:
                tipo_doc = 80  # CUIT
                nro_doc = int(cliente.cuit.replace('-', ''))
            else:
...
# ============================================================================
# FUNCIÓN AUXILIAR: Autorizar comprobante en AFIP
# ============================================================================

def autorizar_comprobante_afip1(datos):
    """
    Autorizar un comprobante (factura o NC) en AFIP.
    
    Esta función debe adaptarse según tu implementación actual de AFIP.
    """
    try:
        # IMPORTANTE: Esta es una implementación genérica
        # Deberás adaptarla a tu código actual de AFIP
        
        # 1. Obtener token de acceso (si no lo tenés cacheado)
        token = obtener_token_afip()  # Tu función actual
        
        # 2. Preparar request para AFIP
        ws = conectar_wsfe()  # Tu función actual que conecta al Web Service
        
        # 3. Armar el request según el tipo de comprobante
        comprobante = {
            ...
            'CbteFch': datos['fecha'],
            'ImpTotal': datos['importes']['total'],
            'ImpTotConc': 0,
            'ImpNeto': datos['importes']['neto'],
            'ImpOpEx': 0,
            'ImpIVA': datos['importes']['iva'],
            'ImpTrib': 0,
            'MonId': 'PES',
            'MonCotiz': 1,
            'DocTipo': datos['cliente']['documento_tipo'],
            'DocNro': datos['cliente']['documento_numero'],
            'Concepto': 1
        }
        
        # 4. Si es NC, agregar comprobante asociado
        if 'comprobante_asociado' in datos:
            comprobante['CbtesAsoc'] = [{
                'Tipo': datos['comprobante_asociado']['tipo'],
                'PtoVta': datos['comprobante_asociado']['punto_venta'],
                'Nro': datos['comprobante_asociado']['numero']
            }]
        
        # 5. Enviar a AFIP
        resultado = ws.FECAESolicitar(
            Auth={'Token': token['token'], 'Sign': token['sign'], 'Cuit': CUIT_EMPRESA},
            FeCAEReq={'FeCabReq': {...}, 'FeDetReq': [comprobante]}
        )
        ...
        if resultado.FeCabResp.Resultado == 'A':
            return {
                'success': True,
                'cae': resultado.FeDetResp.FECAEDetResponse[0].CAE,
                'vto_cae': resultado.FeDetResp.FECAEDetResponse[0].CAEFchVto
            }
        else:
            return {
                'success': False,
                'error': resultado.Errors.Err[0].Msg if resultado.Errors else 'Error desconocido'
            }
